...
const CodexTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "vfactor-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__VFACTOR_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__VFACTOR_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "vfactor-codex-text-" });
