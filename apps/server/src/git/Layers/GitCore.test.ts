  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vfactor/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vfactor/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "vfactor/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "vfactor/tmp-working",
          newBranch: "vfactor/feat/session",
        });

        expect(renamed.branch).toBe("vfactor/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "vfactor/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "vfactor/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("vfactor/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vfactor/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vfactor/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vfactor/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "vfactor/tmp-working" });
        ...
        expect(renamed.branch).toBe("vfactor/feat/session-2");
      }),
    );
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, [
            ...
            "add",
            "origin",
            "git@github.com:vFactor-io/vfactor-desktop.git",
          ]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:vFactor-io/vfactor-desktop.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "vfactor/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
