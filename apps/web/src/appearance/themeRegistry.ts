...
function createTheme(seed: ThemeSeed): ThemeDefinition {
  const isDark = seed.appearance === "dark";
  const border = mix(seed.foreground, isDark ? 8 : 9, seed.border);
  const controlBorder =
    seed.controlBorder ?? mix(seed.foreground, isDark ? 11 : 10, seed.background);
  const input = seed.input ?? mix(seed.foreground, isDark ? 8 : 8, seed.background);
  const sidebarBorder = seed.sidebarBorder ?? mix(seed.foreground, isDark ? 7 : 5, seed.background);
