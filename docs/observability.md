...
vFactor has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.vfactor/userdata/logs/server.trace.ndjson`).
...
```bash
export VFACTOR_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export VFACTOR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export VFACTOR_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export VFACTOR_TRACE_MIN_LEVEL=Info
export VFACTOR_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `VFACTOR_OTLP_*`.

macOS app bundle example:

```bash
VFACTOR_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
VFACTOR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
VFACTOR_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/vFactor.app/Contents/MacOS/vFactor"
```
...
```bash
tail -f "$VFACTOR_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$VFACTOR_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$VFACTOR_HOME/userdata/logs/server.trace.ndjson"
```
...
- `VFACTOR_OTLP_TRACES_URL` was not set
...
- `VFACTOR_TRACE_FILE`: override trace file path
- `VFACTOR_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `VFACTOR_TRACE_MAX_FILES`: rotated file count, default `10`
- `VFACTOR_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `VFACTOR_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `VFACTOR_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `VFACTOR_OTLP_TRACES_URL`: OTLP trace endpoint
- `VFACTOR_OTLP_METRICS_URL`: OTLP metric endpoint
- `VFACTOR_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `VFACTOR_OTLP_SERVICE_NAME`: service name, default `t3-server`
