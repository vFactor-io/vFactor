...
This guide is for people who want to use more than one Claude setup in vFactor.
...
In vFactor Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means vFactor uses your normal home directory.
...
Then add another Claude provider in vFactor:
...
vFactor only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so vFactor keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in vFactor Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. vFactor stores the value as a server secret and does not
...
vFactor does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in vFactor.
