...
This guide is for people who want to use more than one Codex account in vFactor.
...
In vFactor Settings, name it something obvious:
...
In vFactor Settings, add another Codex provider:
...
vFactor shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, vFactor treats it
