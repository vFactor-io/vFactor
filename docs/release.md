...
This document covers the stable desktop release workflow.
...
- Triggers:
  - push tag matching `v*.*.*`
  - manual `workflow_dispatch` with a release version
...
- Publishes one GitHub Release with all produced files.
  ...
  - Automatically generated release notes are pinned to the previous stable tag.
...
- Repository slug source:
  - `VFACTOR_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `VFACTOR_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
