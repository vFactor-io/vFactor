...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit VFACTOR_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "VFACTOR_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid VFACTOR_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults VFACTOR_HOME to ~/.vfactor when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VFACTOR_HOME, path.resolve(NodeOS.homedir(), ".vfactor"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VFACTOR_HOME, path.resolve("/tmp/custom-t3"));
        assert.equal(env.VFACTOR_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.VFACTOR_NO_BROWSER, "1");
        assert.equal(env.VFACTOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.VFACTOR_LOG_WS_EVENTS, "1");
        assert.equal(env.VFACTOR_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            VFACTOR_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
        ...
        assert.equal(env.VFACTOR_MODE, "web");
        assert.equal(env.VFACTOR_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VFACTOR_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VFACTOR_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            VFACTOR_PORT: "13773",
            VFACTOR_MODE: "web",
            VFACTOR_NO_BROWSER: "0",
            VFACTOR_HOST: "0.0.0.0",
        ...
        assert.equal(env.VFACTOR_MODE, undefined);
        assert.equal(env.VFACTOR_NO_BROWSER, undefined);
        assert.equal(env.VFACTOR_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VFACTOR_PORT, "13773");
