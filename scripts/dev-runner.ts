...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".vfactor"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("VFACTOR_PORT_OFFSET"),
  devInstance: optionalStringConfig("VFACTOR_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid VFACTOR_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `VFACTOR_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric VFACTOR_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed VFACTOR_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      VFACTOR_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.VFACTOR_PORT = String(serverPort);
      ...
      delete output.VFACTOR_MODE;
      delete output.VFACTOR_NO_BROWSER;
      delete output.VFACTOR_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.VFACTOR_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.VFACTOR_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.VFACTOR_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.VFACTOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.VFACTOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.VFACTOR_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.VFACTOR_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.VFACTOR_MODE = "web";
      delete output.VFACTOR_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read VFACTOR_PORT_OFFSET/VFACTOR_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.VFACTOR_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.VFACTOR_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all vFactor data (equivalent to VFACTOR_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("VFACTOR_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to VFACTOR_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("VFACTOR_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to VFACTOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("VFACTOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to VFACTOR_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("VFACTOR_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to VFACTOR_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("VFACTOR_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to VFACTOR_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("VFACTOR_PORT")),
  ),
